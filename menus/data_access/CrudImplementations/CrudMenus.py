...
class CrudMenus(CrudInterface):
    ...
    def create(self, object_instance: Menu):
        #todo verif type plats -> a transformer de json vers str 
        sql = "INSERT INTO MENU (utilisateur_id, plats, date_creation, date_modification) VALUES (?, ?);"
        self.data_access.execute(sql, (object_instance.utilisateur_id,
                                       str(object_instance.plats),
    ...
    def read(self, object_id: int):
        ...
        #list obj plat 
        if result:
            menu = Menu(id=result[0], utilisateur_id=result[1], plats=result[2], date_creation=result[2], date_modification=result[2])
            return menu
        else:
            return None
    ...
    def update(self, object_id: int, object_instance: Menu):
        ...
        sql = "UPDATE MENU SET utilisateur_id = ?, plats = ?, date_creation = ?, date_modification = ? WHERE ID = ?;"
        self.data_access.execute(sql, (object_instance.utilisateur_id, object_instance.plats,
